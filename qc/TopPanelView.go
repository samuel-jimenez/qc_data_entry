...
func NewTopPanelView(parent windigo.Controller) *TopPanelView {
	...
	testing_lot_field.OnSelectedChange().Bind(func(e *windigo.Event) {
		...
		view.QC_Product.Container_type = product.DiscreteFromInt(DB.CONTAINER_SAMPLE)
...
func (view *TopPanelView) RefreshSize(font_size int) {
	...
	top_subpanel_height = top_spacer_height + num_rows*(GUI.PRODUCT_FIELD_HEIGHT+GUI.INTER_SPACER_HEIGHT) + GUI.BTM_SPACER_HEIGHT
